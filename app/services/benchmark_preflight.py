...
async def collect_benchmark_metadata(db: AsyncSession | None, database_url: str) -> dict[str, Any]:
    ...
    meta: dict[str, Any] = {
        ...
        "expected_kerala_corpus_rows": expected_kerala_corpus_rows(),
    ...
    meta["kerala_corpus_seeded"] = count >= max(expected - 2, 50)

    if not meta["kerala_corpus_seeded"]:
        meta["warnings"].append(
...
def enforce_kerala_corpus_preflight(meta: dict[str, Any], *, require: bool) -> None:
    ...
    if not meta.get("kerala_corpus_seeded"):
        raise SystemExit(
            ...
            "Run scripts/seed_kerala_language_aliases.py before benchmarking."
        )
