...
Pipeline (each step is fully independent — failure in one layer does not abort the others):
    ...
    L1  Redis cache lookup (positive + negative)
...
Returned shape per match::

    {
      "hsn_code", "description", "score", "method",
      "gst_rate", "category", "chapter", "layer"
    }
"""
...
# Per-layer hard timeouts (seconds) — read from settings so they can be tuned per env.
LAYER_TIMEOUTS = {
...
async def _layer_inverted(db: AsyncSession, queries: Sequence[str], limit: int) -> list[dict]:
    """Runs the tsvector lookup for each query variant and merges the top results."""
    seen: dict[str, dict] = {}
    for q in queries[:4]:
...
async def _layer_prefix(db: AsyncSession, query: str, limit: int) -> list[dict]:
    ...
    if len(digits) < 2 or len(digits) > 8:
        return []
    rows = (
        await db.execute(_PREFIX_SQL, {"pat": digits + "%", "limit": int(limit)})
    ).mappings().all()
    out: list[dict] = []
    for r in rows:
...
async def _layer_verified(db: AsyncSession, query: str, limit: int) -> list[dict]:
    ...
    try:
        ...
        rows = (
            await db.execute(_VERIFIED_SQL, {"q": q, "limit": int(limit)})
        ).mappings().all()
    except Exception as exc:
...
async def _bounded(name: str, coro, timeout: float, layers: list[LayerTrace]) -> list[dict]:
    ...
    try:
        ...
        layers.append(LayerTrace(name=name, ms=(time.perf_counter() - started) * 1000, candidate_count=0, used=False, error="timeout"))
        return []
    except Exception as exc:
        layers.append(LayerTrace(name=name, ms=(time.perf_counter() - started) * 1000, candidate_count=0, used=False, error=str(exc)[:120]))
        return []
    layers.append(LayerTrace(name=name, ms=(time.perf_counter() - started) * 1000, candidate_count=len(rows)))
    return rows
...
async def multi_search(
    ...
    *,
    ...
    try:
        ...
        layers.append(LayerTrace(name="L0_alias", ms=(time.perf_counter() - t0) * 1000, candidate_count=len(expanded.expansions)))
    except Exception as exc:
        layers.append(LayerTrace(name="L0_alias", ms=(time.perf_counter() - t0) * 1000, candidate_count=0, used=False, error=str(exc)[:120]))
    ...
    if not bypass_cache:
        ...
        try:
            ...
            cached = None
        layers.append(LayerTrace(name="L1_cache", ms=(time.perf_counter() - t1) * 1000, candidate_count=1 if cached else 0))
    ...
    verified_task = _bounded("L6_verified_products", _layer_verified(db, eng_q, top_k * 2), LAYER_TIMEOUTS["verified"], layers)

    inv_rows, fuzzy_rows, faiss_rows, prefix_rows, verified_rows = await asyncio.gather(
        inv_task, fuzzy_task, faiss_task, prefix_task, verified_task
    )

    # Hint codes from alias direct-HSN hits get a small boost.
    ...
    # Cache positive + negative results so we don't keep retrying empty queries.
...
# ── Helper queries used by /search/categories and /search/by-language ─────────
...
async def warmup(db: AsyncSession) -> None:
    """Call from app lifespan: pre-load alias index + ping common indexes."""
    try:
